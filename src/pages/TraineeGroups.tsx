...
import { Search, Plus, Users, Calendar, Play, MoreHorizontal, Clock, MapPin, Trophy, Eye, AlertCircle } from "lucide-react";
...
import { getTraineeGroups, searchTraineeGroups, deleteTraineeGroup } from "@/services/traineeGroup.services";
...
const getLevelColor = (level: string) => {
  switch (level) {
    ...
    case "Advanced": return "bg-success/10 text-success hover:bg-success/20";
    case "Mixed": return "bg-warning/10 text-warning hover:bg-warning/20";
...
export default function TraineeGroups() {
  ...
  const searchFn = useCallback(
    (term: string, page: number, pageSize: number) => searchTraineeGroups(term, page, pageSize),
    [],
  );

  const { items: groups, loading, term, setTerm, page, setPage, totalPages, refresh } =
    useEntitySearch<ListTraineeGroupDto>({ listFn, searchFn, pageSize: 9 });

  const handleGenerate = (group: ListTraineeGroupDto) => {
  ...
  const statsData = [
    { title: "Total Groups", value: loading ? "—" : String(groups.length), icon: Users },
    { title: "Total Schedules", value: "—", icon: Calendar },
    { title: "Avg Duration", value: loading || groups.length === 0 ? "—" : `${Math.round(groups.reduce((s, g) => s + g.durationInMinutes, 0) / groups.length)} min`, icon: Clock },
    { title: "Total Trainees", value: loading ? "—" : String(groups.reduce((s, g) => s + (g.traineesCount ?? 0), 0)), icon: Trophy },
  ];
  ...
  return (
    <div className="space-y-8">
      ...
      {loading ? (
        ...
        <Card className="card-athletic">
          <CardContent className="p-12 flex flex-col items-center gap-3 text-center">
            ...
            <p className="text-muted-foreground text-sm">Try a different search term or create a new group.</p>
          </CardContent>
        </Card>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
                ...
                <div className="flex items-start justify-between">
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      ...
                      <DropdownMenuItem onClick={() => navigate(`/trainee-groups/${group.id}`)}>View Details</DropdownMenuItem>
                      <DropdownMenuSeparator />
                      <DropdownMenuItem
                        className="text-destructive"
                        onClick={() => setTimeout(() => handleDelete(group.id), 100)}
                      >
                        Delete
                      </DropdownMenuItem>
                    </DropdownMenuContent>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2 pt-3 border-t border-border">
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-muted-foreground">Trainees enrolled</span>
      ...
      <GenerateSessionsModal
        ...
        groupName={selectedGroup?.sportName}
        schedules={[]}
      />
    </div>
  );
