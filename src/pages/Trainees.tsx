...
const trainees = [
  ...
  { id: 2, name: "Marcus Thompson", email: "marcus.t@email.com", phone: "+1 (555) 234-5678", age: 14, joinDate: "2024-02-20", sport: "Soccer", level: "Beginner", attendanceRate: 88, branch: "North Side", status: "Active" },
  { id: 3, name: "Sofia Rodriguez", email: "sofia.rodriguez@email.com", phone: "+1 (555) 345-6789", age: 17, joinDate: "2023-09-10", sport: "Tennis", level: "Advanced", attendanceRate: 92, branch: "East Branch", status: "Active" },
  { id: 4, name: "David Kim", email: "david.kim@email.com", phone: "+1 (555) 456-7890", age: 15, joinDate: "2024-03-05", sport: "Swimming", level: "Intermediate", attendanceRate: 97, branch: "Downtown", status: "Active" },
  { id: 5, name: "Emma Davis", email: "emma.davis@email.com", phone: "+1 (555) 567-8901", age: 13, joinDate: "2024-01-30", sport: "Volleyball", level: "Beginner", attendanceRate: 85, branch: "North Side", status: "On Hold" },
];
...
export default function Trainees() {
  ...
  const getLevelColor = (level: string) => {
    switch (level) { case "Beginner": return "bg-secondary/10 text-secondary hover:bg-secondary/20"; case "Intermediate": return "bg-primary/10 text-primary hover:bg-primary/20"; case "Advanced": return "bg-success/10 text-success hover:bg-success/20"; default: return "bg-muted"; }
  };
  ...
  return (
    <div className="space-y-8">
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <Badge variant="secondary" className="bg-success/10 text-success hover:bg-success/20 mt-2"><TrendingUp className="h-3 w-3 mr-1" />{stat.change}</Badge>
                </div>
        ...
        <CardContent>
          <div className="flex flex-col sm:flex-row gap-4">
            <div className="relative flex-1">
              ...
              <Input placeholder="Search trainees by name or email..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10" />
            </div>
            <Button variant="outline" className="flex items-center gap-2"><Filter className="h-4 w-4" />Filter by Sport</Button>
            <Button variant="outline" className="flex items-center gap-2"><Filter className="h-4 w-4" />Filter by Status</Button>
          </div>
        </CardContent>
      </Card>
              ...
              <TableBody>
                {paginatedData.map((trainee) => (
                  <TableRow key={trainee.id} className="hover:bg-muted/50">
                    ...
                    <TableCell><div className="space-y-2"><Badge variant="outline" className="font-medium">{trainee.sport}</Badge><Badge variant="secondary" className={getLevelColor(trainee.level)}>{trainee.level}</Badge></div></TableCell>
                    <TableCell><div className="flex items-center gap-2 text-sm"><MapPin className="h-3 w-3 text-muted-foreground" />{trainee.branch}</div></TableCell>
      ...
      <TraineeFormModal open={modalOpen} onOpenChange={setModalOpen} onSuccess={handleRefresh} />
    </div>
  );
}
